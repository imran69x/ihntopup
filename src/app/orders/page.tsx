          ...
          <div className="flex flex-col items-end gap-2">
            <p className="font-bold text-lg text-primary flex items-center gap-1">
              {order.paymentMethod === 'CoinFund' ? (
                <>
                  <Image src="/coin-icon.png" alt="Coin" width={18} height={18} className="w-[18px] h-[18px]" />
...
export default function OrdersPage() {
  ...
  // Filter out reseller orders - only show regular orders on main orders page
  const orders = useMemo(() => {
    if (!allOrders) return [];
    return allOrders.filter(order => !order.isResellerProduct);
  }, [allOrders]);
